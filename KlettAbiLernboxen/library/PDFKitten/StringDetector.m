...
{
    if (![delegate detectorShouldStartMatching:self])
        return @"";
    
    
//    LOG(@"font: %@", [font class]);
    
    uint len = CGPDFStringGetLength(string);
//    LOG(@"string length: %d", len);
...
//    if (len < 2) return @"";
    ...
    if (!cidString) LOG(@"empty cidString");
 	
	// Use Unicode string to compare with user input.
	NSString *unicodeString = [[font stringWithPDFString:string] lowercaseString];

    LOG(@"unicodeString: %@", unicodeString);

    if (!unicodeString) {
        LOG2(@"empty unicodeString");
        counter_incI(@"empty unicodeString", 1);
        LogM(@"empty_unicodeString", @"dropped\n");
        unicodeString = @"";
    }
