...
#define WORKAROUND_SCALE_RATIO      0.98
#define SCALE_RATIO         1.15
//#define SCALE_RATIO 1.0
//#define SCALE_RATIO         1.0



static CGColorRef __PDFViewPatternColor = nil;
...
//        // -- DELETE
        ...
        CGRect cropRect = CGRectIntegral(CGPDFPageGetBoxRect(self.pageRef, kCGPDFTrimBox));
        CGRect pageRect = cropRect;
        pageRect = CGRectApplyAffineTransform(pageRect, CGAffineTransformMakeScale(WORKAROUND_SCALE_RATIO, WORKAROUND_SCALE_RATIO));
        pageRect = CGRectApplyAffineTransform(pageRect, CGAffineTransformMakeScale(1.0, SCALE_RATIO));
...
- (CGColorRef) patternColor {
    ...
    if (!__PDFViewPatternColor) {
        
        __PDFViewPatternColor = CGColorRetain([UIColor colorWithPatternImage:_backgroundImage].CGColor);
    }
    
    return __PDFViewPatternColor;
}
...
- (void) setPageRef:(CGPDFPageRef)thePageRef {
    ...
    pageRef = thePageRef;
    CGRect pageRect = CGRectIntegral(CGPDFPageGetBoxRect(thePageRef, kCGPDFTrimBox));
...
//    NSLog(@"_backgroundImage: %@", _backgroundImage);
    CGContextSetFillColorWithColor(context, [self patternColor]);
...
//    [backgroundImage drawInRect:CGContextGetClipBoundingBox(context) blendMode:kCGBlendModeNormal alpha:1.0];
    ...
    CGAffineTransform transform = CGPDFPageGetDrawingTransform(self.pageRef, kCGPDFTrimBox, pageRect, 0, true);
...
//    CGAffineTransform transform2 = CGPDFPageGetDrawingTransform(self.pageRef, kCGPDFMediaBox, layer.bounds, 0, true);
    ...
    CGFloat scaleRatio = SCALE_RATIO;
    CGFloat translationX = (layer.bounds.size.width - layer.bounds.size.width * scaleRatio) * 0.5;
    
    transform = CGAffineTransformConcat(transform, CGAffineTransformMakeScale(scaleRatio, scaleRatio));
